...
function SceneFlowNodeImpl({ id }: NodeProps) {
  const { projectId, activeId, onEdit } = useFlowCallbacks();
  ...
  return (
    <div
      ...
      className={cn(
        "group/node relative flex select-none flex-col overflow-hidden rounded-[var(--radius-card)] border bg-white transition-shadow duration-150",
        isActive
